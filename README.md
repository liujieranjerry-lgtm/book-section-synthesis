...
[查看完整输出 →](references/examples/expected-output.md)
...
需要固定结构的笔记时，使用 [templates/section-note.md](templates/section-note.md)。
...
详细规则见 [SKILL.md](SKILL.md)、[references/text-types.md](references/text-types.md) 和 [references/fidelity.md](references/fidelity.md)。
...
官方 `skill-installer` 命令见 [30 秒开始](#30-秒开始)。下面是其他安装方式。
...
具体 skills 目录以对应 agent 的文档为准。`agents/openai.yaml` 是 Codex 的界面元数据，其他宿主可以忽略。
...
模型实际读取的是 `SKILL.md`、`references/` 和需要时的 `templates/section-note.md`；其余文件用于开发、评测和发布。
...
**支持 Claude Code 或其他 agent 吗？** 支持。把仓库克隆到对应 skills 目录即可；`agents/openai.yaml` 是 Codex 的可选元数据。
